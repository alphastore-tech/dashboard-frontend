...
/** 공통 fetcher: LS 잔고는 POST로 조회 */
const fetcher = (url: string) => fetch(url, { method: 'POST' }).then((r) => r.json());

/** 5초마다 잔고 재호출 */
export default function useLsBalance(options: { polling?: boolean } = {}) {
  const { polling = true } = options;
  const marketOpen = isMarketOpenKST(900, 1545);

  return useSWR('/api/ls/balance', fetcher, {
    refreshInterval: polling && marketOpen ? 5_000 : 0, // 5 000 ms = 5 s
